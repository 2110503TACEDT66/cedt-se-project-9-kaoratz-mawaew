...
const mockRestaurantItem: RestaurantItem = {
  ...
  __v: 0,
  imageUrl:
    "https://images.unsplash.com/photo-1603361513137-219be38712ed?q=80&amp;",
...
describe("Card Component", () => {
  it("renders the card with correct information", () => {
    ...
    const addressElement = screen.getByText(
      `${mockRestaurantItem.address}, ${mockRestaurantItem.district}, ${mockRestaurantItem.province}, ${mockRestaurantItem.postalcode}`
    );
  ...
  it('navigates to restaurant details page on "Details" button click', () => {
    ...
    expect(detailButton).toHaveAttribute(
      "href",
      `/restaurant/${mockRestaurantItem.id}`
    );
  });
  ...
  it('navigates to restaurant details page on "Reserve" button click', () => {
    ...
    expect(detailButton).toHaveAttribute(
      "href",
      `/reserve?id=${mockRestaurantItem.id}&name=${mockRestaurantItem.name}`
    );
  });
});
